...
public class MergeSort {
    ...
    private static <Item extends Comparable> Queue<Queue<Item>>
        ...
        for (Item p : items) {
            Queue<Item> addOne = new Queue<>();
            addOne.enqueue(p);
            SingleItemQueue.enqueue(addOne);
        }
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
            Queue<Item> items) {
        Queue<Queue<Item>> helpQueue = makeSingleItemQueues(items);
        while(helpQueue.size() > 1) {
            helpQueue.enqueue(mergeSortedQueues(helpQueue.dequeue(), helpQueue.dequeue()));
        }
        return helpQueue.dequeue();
    }
}
